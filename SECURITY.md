...
We take the security of Nexus — the Enterprise Intelligence Framework — and its users seriously. This document
...
typically the latest two minor versions.
...
- The published `veloxs-nexus` package on PyPI.
...
- Reports that depend on attacker-controlled `nexus.json` or layer configs.
...
- **`pydantic`** — the typed config/schema contract every layer exposes.
  Any control plane can introspect these models to render and validate
  configuration automatically; replacing it with hand-rolled parsing
  would weaken validation, not strengthen it.
