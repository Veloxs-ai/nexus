...
The engagement layer is configured in [configs/engagement.yaml](configs/engagement.yaml).
...
integration:
  ...
  guardrails_config: ../orchestration-guardrails/configs/guardrails.yaml
  mode: subprocess_cli
```
...
```bash
PYTHONPATH=src python -m nexus_experience.cli validate-config configs/engagement.yaml
```

## Ask Through The Engagement Layer

```bash
experience ask configs/engagement.yaml "What does the security policy say about MFA?" --channel assistant
```

Without package installation:

```bash
PYTHONPATH=src python -m nexus_experience.cli ask configs/engagement.yaml "What does the security policy say about MFA?" --channel assistant
```
