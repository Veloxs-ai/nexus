...
from .exporters import validate_exporters
from .models import MetricKind, Severity
...
def build_parser() -> argparse.ArgumentParser:
    ...
    validate_config = commands.add_parser(
        "validate-config", help="Load and validate an observability config."
    )
    ...
    record_metric.add_argument(
        "--kind", type=MetricKind, choices=list(MetricKind), default=MetricKind.GAUGE
    )
    ...
    evaluate_alerts = commands.add_parser(
        "evaluate-alerts", help="Evaluate alert rules over recorded events."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate-config":
        print(
            f"Loaded {len(config.services)} monitored services "
            f"and {len(config.exporters)} exporters."
        )
