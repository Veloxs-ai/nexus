...
> This layer provides the **Observability** capability.
...
Monitoring is configured in [configs/observability.json](configs/observability.json).
...
```bash
PYTHONPATH=src python -m nexus_observability.cli validate-config configs/observability.json
```
