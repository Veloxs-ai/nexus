...
> A layered, open-source enterprise AI platform that turns fragmented data
...
- **Open-source integrators** who want a well-typed, well-tested Python
  baseline they can fork and adapt.
...
2. Pick an open issue or propose a new feature behind one of the documented
   extension points.
...
4. Add or extend tests in the affected layer's `tests/` directory. PRs
   should leave every layer's pytest green.
5. Format / lint with `ruff` (already configured in each `pyproject.toml`).
6. For security-affecting changes, please open a draft PR and request
...
- **License**: choose one (Apache-2.0 and MIT are both common for
  infrastructure libraries; if the project will be used commercially by
  enterprises, Apache-2.0 is the safer default because of its explicit
  patent grant). Add a top-level `LICENSE` file and update this section
  before tagging your first public release.
- **Maintainers**: list your maintainer team here with contact channels
  (GitHub handles, email, Slack/Discord).
- **Support**: clarify what you offer (community-only, best-effort, paid
  support tiers).
