# Nexus — Integrator Guide

> The **Enterprise Intelligence Framework** — a layered, open-source framework
> that turns fragmented enterprise data into secure, grounded, context-aware
> intelligence, usable from any Python project as a library, a CLI, or a REST
> service.
...
Nexus is the **Enterprise Intelligence Framework**, built as seven loosely-coupled
...
- **Security and compliance teams** evaluating how RBAC, tenant isolation,
  PII masking, and audit logging can be made first-class in an AI framework.
...
Nexus is open source under the Apache License 2.0 and welcomes contributions.
...
4. Add or extend tests in the affected layer's `tests/` directory. Every
   layer's pytest suite should be green.
5. Format and lint with `ruff` (configured in each `pyproject.toml`).
6. Sign off your commits for the [DCO](../CONTRIBUTING.md#developer-certificate-of-origin)
   (`git commit -s`). There is no CLA.
7. New `.py` files carry the Apache-2.0 SPDX header — CI enforces this.
...
- **License**: [Apache License 2.0](../LICENSE). Copyright © 2026 Veloxs AI
  Inc. You are free to use, modify, and distribute Nexus, including
  commercially, subject to the terms of the license. See [NOTICE](../NOTICE)
  for attribution and third-party dependency information.
- **Trademarks**: Apache-2.0 §6 grants no rights in the Nexus or Veloxs AI
  names, logos, or branding. You may state truthfully that your software is
  built on Nexus; you may not imply endorsement. See [NOTICE](../NOTICE).
- **Maintainers**: see [MAINTAINERS.md](../MAINTAINERS.md).
- **Getting help**: see [SUPPORT.md](../SUPPORT.md) for where to ask
  questions and what response to expect.
