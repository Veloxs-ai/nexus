...
The codebase is split across three major repositories/directories:
...
2. **`veloxs-platform`** *(Control Plane & UI)*:
   ...
   - Utilizes Pydantic schemas from the `nexus` layers to auto-generate UI configuration forms.
   - Orchestrates execution of Nexus layers via CLI/subprocess calls and maps secrets (KMS) dynamically.

3. **`nexus-veloxs-ai`** *(Documentation & Assets)*:
   - Host for static documentation, design layouts, architecture guides, and marketing assets.
...
Below is a complete breakdown of what can be tested in each part of the `nexus` repository:
...
* **Metadata Extraction (`test_enrichment.py`)**: Asserts dynamic keyword extraction, schema enforcement, and parent-child file linking.
...
1. **Multi-Tenant SSO & API Key RBAC**:
   ...
   - Verifies Owner role permissions vs. Viewer role permissions (e.g. read-only projects list).
