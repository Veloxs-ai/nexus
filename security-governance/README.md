...
Security policies are configured in [configs/security.yaml](configs/security.yaml).
...
```bash
PYTHONPATH=src python -m nexus_security.cli validate-config configs/security.yaml
```
...
```bash
security check-access configs/security.yaml analyst read:data tenant-a tenant-b
```
